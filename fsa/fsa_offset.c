...
/*     while((size_t)pool%WORD_SIZE != 0)
   ...
   pool_handler->next_free_offset = sizeof(fsa_t);
...
void FSAFree(fsa_t *pool, void *block_to_free)
{
    ...
    *(size_t *)block_to_free = pool->next_free_offset;

    /*the offset is the address of the to free minus pool */
    pool->next_free_offset = ((char*)block_to_free - (char*)pool);
...
void *FSAAlloc(fsa_t *pool)
{
    ...
    return_address = (char*)pool + pool->next_free_offset;

    /* the value of tmp_address is the value of next free address */
    pool->next_free_offset = *(size_t*)return_address;
